...
export const isWinningMove = (move: GameMove): boolean => {
  ...
  return (
    (destination.y === 0 && player === PlayerColor.Black) ||
    (destination.y === 7 && player === PlayerColor.White)
  );
};
...
export const createMove = (
  ...
  if (isNull(state.currentPaw)) {
    throw new Error(
      "Tile should not be playable if no paw is currently selected"
    );
  }
  return new GameMove(
    getTileFromClick(state.currentPaw.x, state.currentPaw.y),
    destinationTile,
    state.currentPaw
  );
};
