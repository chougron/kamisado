 ...
 */
export const getPawFromTile = (tile: ITile, state: GameState): IPaw | undefined => {
 ...
 * @param state The current state of the game
 */
export const getPawFromCoordinates = (x: number, y: number, state: GameState): IPaw | undefined => {
 ...
 * @param state The current state of the game
 */
export const getPawByColors = (color: TileColor, playerColor: PlayerColor, state: GameState): IPaw => {
  const foundPaw = state.paws.find((paw: IPaw) => paw.playerColor === playerColor && paw.color === color);
...
export const isCurrentPaw = (paw: IPaw, state: GameState): boolean => {
  return !!state.currentPaw && paw.color === state.currentPaw.color && paw.playerColor === state.currentPlayer;
};
...
export const movePaw = (paws: IPaw[], move: GameMove): IPaw[] => {
  ...
  return paws.map((paw: IPaw) => {
    if (!!currentPaw && paw.color === currentPaw.color && paw.playerColor === currentPaw.playerColor) {
      paw.x = destination.x;
      paw.y = destination.y;
    }
    return paw;
